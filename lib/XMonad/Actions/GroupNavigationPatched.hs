...
-- |
-- Module      : XMonad.Actions.GroupNavigationPatched
...
nextMatchOrDo dir qry act =
  orderedWindowList dir
    >>= focusNextMatchOrDo qry act

-- Produces the action to perform depending on whether there's a
-- matching window
-- getNextMatch :: Query Bool -> Seq Window -> X (Maybe Window)
getNextMatch :: Query Bool -> Direction -> X (Maybe Window)
getNextMatch qry dir = orderedWindowList dir >>= findM (runQuery qry)
...
data HistoryDB
  = HistoryDB
      (Maybe Window) -- currently focused window
